...
export default function DashboardPage() {
  ...
  const loadStats = async () => {
    setPromptStats({ total: 0, published: 0, drafts: 0, totalViews: 0, totalLikes: 0 })
  ...
  const stats = [
    ...
    { label: 'Likes Received', value: promptStats.totalLikes.toString(), icon: FiHeart, color: 'from-red-500 to-rose-500' },
    { label: 'Saved', value: savedCount.toString(), icon: FiBookmark, color: 'from-indigo-500 to-purple-500' },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-black">
      <div className="container mx-auto px-4 py-8">
        <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-2xl p-8 mb-8 border border-purple-500/20">
          ...
          <p className="text-gray-300 text-lg">Manage your prompts, track performance, and engage with the community.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-12">
          {stats.map((stat) => {
            const Icon = stat.icon
            return (
              <div key={stat.label} className="bg-gray-900/50 backdrop-blur-sm rounded-lg p-4 border border-gray-800 hover:border-gray-700 transition">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quickActions.map((action) => {
            ...
            return (
              <Link key={action.title} href={action.href} className="bg-gray-900/50 backdrop-blur-sm rounded-lg p-6 border border-gray-800 hover:border-gray-700 transition group">
